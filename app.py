...
# MUHIM:
# Gunicorn/hosting app.py ni import qilganda ham database yaratiladi.
try:
    init_db()
...
def fazer(method, path, **kwargs):
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"FazerCards API xatosi: {str(e)}"
        }

    except Exception as e:

        print("FAZER UNKNOWN ERROR:", e)
...
def aktiv(action, **params):
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"AktivSIM API xatosi: {str(e)}"
        }

    except Exception as e:

        print("AKTIV UNKNOWN ERROR:", e)
...
def price_usd_to_uzs(usd):
    ...
    return int(
        (
            Decimal(str(usd)) * USD_UZS + MARKUP_UZS
        ).quantize(
            Decimal("1"),
